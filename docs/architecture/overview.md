...
The Tabby rail's **Run report** action does not call an assistant endpoint. It sends the current address list and filters through `useCompare`, which calls the public dashboard analysis, neighborhood, incident-detail, and (for two or more places) comparison endpoints, then freezes the response into an expanded, result-focused `AnalysisCard` in the same rail. Canonical reports requested through the assistant open in that same state as soon as they land, so the reference, type, and temporal sections are not hidden behind a second action. Row-level incident memberships stay out of the on-screen and printable report presentation; the report's ZIP data package carries them separately in `records.csv`. The report is focused at its heading and owns the rail's remaining height until it is collapsed; the context/composer, follow-up chips, and conversation then return. Passive returning-session auto-runs remain compact so they do not take over the pane without an explicit request. The rail keeps one live client-generated report card, so another direct run replaces that card instead of stacking a duplicate “previous analysis”; assistant-produced cards remain part of the conversation.
...
card restores the previous width. A direct report enters that same readable-width,
