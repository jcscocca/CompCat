...
Most primary keys are UUID strings (`String(36)`, generated by `app/schemas.new_id`).
`SessionActivity` is the exception: its one-way `user_id_hash` is the primary key. Every
user-scoped table carries that hashed session identity (see §2). Sources: `app/models.py`.
...
persistent foreign-key `IntegrityError` aborts the sweep instead of returning misleading
success counts.
...
if not settings.raw_upload_retention:
    ...
    session.execute(delete(ImportBatch).where(...id...))
    session.commit()
```
