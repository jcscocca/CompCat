...
def _proxy_header_ip(headers, *, trust_x_forwarded_for: bool = False) -> str | None:
    """Client IP from explicitly trusted proxy headers.

    CF-Connecting-IP is the default trusted signal because Cloudflare overwrites it at
    its edge. X-Forwarded-For is considered only under its separate opt-in: its meaning
    depends on the entire proxy chain, and blindly falling back to it lets callers mint
    buckets whenever the immediate edge preserves inbound XFF.
...
# Paths exempt from the burst tier: static assets, the SPA shell, and docs.
# Tiles and health probes have dedicated finite buckets below.
...
async def _send_rate_limit(send, wait: float) -> None:
...
def _client_ip(scope, settings, headers: dict[str, str]) -> str:
    ip = "unknown"
...
def _pmtiles_size(settings, path: str) -> int | None:
    """Return a requested PMTiles artifact's size without escaping tiles_dir."""
    root = Path(settings.tiles_dir).resolve()
    candidate = (root / path.removeprefix("/tiles/")).resolve()
    try:
        ...
        if path.startswith("/tiles/"):
            ...
            if settings.rate_limit_enabled:
                wait = get_rate_limiter().try_take(
                    "tiles",
                    ip,
                    capacity=settings.rate_limit_tiles_per_minute,
                    per_seconds=60.0,
                )
                if wait > 0:
