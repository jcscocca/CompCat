...
describe("placeDefaults", () => {
  ...
  it("falls back to the coordinates, never a fixed name", () => {
    expect(labelOrDefault("", { latitude: 47.6012344, longitude: -122.3312344 })).toBe("Pin at 47.601234, -122.331234");
    expect(labelOrDefault("   ", { latitude: 47.6012344, longitude: -122.3312344 })).toBe("Pin at 47.601234, -122.331234");
  });

  it("renders coordinates to six decimals", () => {
    expect(coordinateLabel(47.6, -122.3)).toBe("Pin at 47.600000, -122.300000");
  });
